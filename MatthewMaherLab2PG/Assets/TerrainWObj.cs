...
public class TerrainWObj : MonoBehaviour
{
    ...
    public GameObject thingPrefab2;
    ...
    Color[] cTerrain = new Color[] { Color.clear, Color.white, Color.cyan };

    [SerializeField]
    private GameObject thingPrefab;
    public GameObject playerPrefab;
    public GameObject simpleRefugePrefab;//object to be placed on the terrain
    ...
    void AddThings()
    {
        ...
        for (int i = 0, z = 0; z <= zWidth; z++)
        {
            for (int x = 0; x <= xWidth; x++, i++)
            {
                if (!spawnedPlayer && noiseTex.GetPixel(x,z) == Color.clear)
                {
                    spawnedPlayer = true;
                    thisPos = vertices[i];

                    GameObject thing = Instantiate(playerPrefab, new Vector3(thisPos.x, thisPos.y+4, thisPos.x), Quaternion.identity);
                    thing.transform.position = vertices[i] + 0.45f * Vector3.up * athingHeight / 2;
                    GameObject thing2 = Instantiate(simpleRefugePrefab, new Vector3(thisPos.x, thisPos.y + 4, thisPos.x), Quaternion.identity);
                    thing.transform.position = vertices[i] + 0.45f * Vector3.up * athingHeight / 2;
                }
                //use the color map to filter the terrain
                if (noiseTex.GetPixel(x, z) == Color.white) //only add things in the green territory
                {
                    if (Random.value < 0.05f) //add things randomly
                    {
                        thisPos = vertices[i]; //the current mesh position - you can shift randomly from here if you want
                        //instantiate a thing at thisPos
                        GameObject thing = Instantiate(thingPrefab2, thisPos, Quaternion.identity);
                ...
                if (noiseTex.GetPixel(x,z) == Color.white) //only add things in the green territory
                {               
                    if (Random.value < 0.1f) //add things randomly
                    {
                        thisPos = vertices[i]; //the current mesh position - you can shift randomly from here if you want
                        //instantiate a thing at thisPos
